# Spec: Scenario Editor Infrastructure Selection

## Overview

Enable users to select their deployment target directly in the topology editor, making the target infrastructure a first-class part of the topology configuration workflow.
...
None - this is an additive capability that works alongside existing infrastructure selection.
...
- `scenario-topology-validation` - Infrastructure selection is first validation rule
- `scenario-editor-guidelines` - Guidelines step 1 references infrastructure selection
